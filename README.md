...
메시지에 `.md` 파일 경로가 포함되거나 `.md` 파일을 첨부하면 자동으로 스펙 모드로 진입한다.

```
Discord: docs/feature_spec.md 기반으로 개발해줘
  (또는 MD 파일 첨부)
  → 체크리스트 항목 수 파악
  → 항목이 없으면: Claude가 GDD를 분석하여 구현 체크리스트 자동 생성
  → LOOP (미완료 항목이 없을 때까지):
...
체크박스 항목이 없는 GDD를 올려도 동작한다. Claude가 문서를 분석하여 `## 구현 체크리스트` 섹션을 자동으로 추가한 뒤 스펙 루프를 시작한다.
