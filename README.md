...
| 상황 | 입력 예시 |
|------|----------|
| 일반 개발 명령 | `PlayerController 이동 로직 최적화해줘` |
| MD 스펙 기반 개발 | `docs/feature_spec.md 기반으로 개발 시작해줘` |
| 작업 중 피드백 | `점프 높이를 2배로 줄여줘` (큐에 적재 → 현재 작업 완료 후 자동 반영) |

#### MD 파일 기반 개발 워크플로우

1. 스펙 MD 파일을 Unity 프로젝트의 `docs/` 폴더에 작성
2. Discord에서 해당 파일 경로를 명시하여 명령
3. Claude Code가 파일을 읽고 자율 개발 루프 진행
